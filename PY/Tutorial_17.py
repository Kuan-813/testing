...
import sqlite3
from Tutorial_15 import DatabaseManager
    ...
    try:
        ...
        if user_id:
            ...
            raise HTTPException(
                ...
                detail="Failed to create user. Email already exist."
            )
...
async def update_user(user_id: int, user_update: UserCreate):
    ...
    try:
        ...
        success = db.update_user(user_id, user_update.name, user_update.email, user_update.age)
        if success:
...
async def update_post(post_id: int, post_update: PostUpdate):
    ...
    try:
        ...
        success = db.update_post(post_id, post_update.title, post_update.content)
        if success:
